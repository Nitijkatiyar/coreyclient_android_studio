 ...
 */
public class GetRemindersWebService extends
    ...
    public GetRemindersWebService(Activity activity, ListView listView, TextView noMessages) {
    ...
    protected JSONObject doInBackground(String... params) {
        ...
        url = MessageTabActivityCMN.CMN_SERVER_BASE_URL_DEFINE
                + "getreminders.aspx?token=" + CMN_Preferences.getUserToken(activity) + "&type=" + params[1];
        if (AppConfig.isAppCoreyHealth) {
            url = url + "&patientid=" + GetAllContextWebService.patientsContexts.get(MessageTabActivityCMN.patientPosition).ContextId;
        } else {
            url = url + "&patientid=" + params[0];
        }
    ...
    protected void onPostExecute(JSONObject result) {
        ...
        if (dialog.isShowing()) {
            ...
            if (reminderList.size() > 0) {
                RemindersAdapter adapter = new RemindersAdapter(activity,
                        reminderList, typeId);
