...
public class LookupByPatientName extends BaseActivityCMN implements NetworkCallBack, AsyncTaskCompleteListener<String> {
    ...
    Intent intent;
    String patId;
    RelativeLayout patientSpinnerLayout;
    AsyncTaskCompleteListener<String> asyncTaskCompleteListener;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_lookup_by_patient_name);
        ActivityPackage.setActivityList(this);

        getActionBar().setHomeButtonEnabled(true);
        getActionBar().setTitle("Search by patient name");
        getActionBar().setDisplayHomeAsUpEnabled(true);


        _patName = (EditText) findViewById(R.id.patName);
        _patient = (Spinner) findViewById(R.id.selectPatient);
        _search = (Button) findViewById(R.id.searchPatient);

        patientSpinnerLayout=(RelativeLayout)findViewById(R.id.patientLayout);
        if (getIntent() != null && getIntent().hasExtra("isSearchPatient")) {
            isSearchPatient = getIntent().getBooleanExtra("isSearchPatient", false);

        }
        if(!isSearchPatient){
            patientSpinnerLayout.setVisibility(View.VISIBLE);
        }
        else{
            _patient.setVisibility(View.GONE);
        }

        progressDialog = new ProgressDialog(LookupByPatientName.this);
        ...
        _search.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    networkAsyncTask.startNetworkCall(ThreadTaskIds.GET_LOOKUP_DATA, LookupByPatientName.this);
    ...
    public Object onNetworkCall(int taskId, Object o) throws
        ...
        if (taskId == ThreadTaskIds.GET_LOOKUP_DATA) {
            String url = CMN_Preferences.getBaseUrl(LookupByPatientName.this)
                    + "PatientLookup_s.aspx?token=" + CMN_Preferences.getUserToken(LookupByPatientName.this);

            NetworkTools networkTools = NetworkTools.getInstance();
            if (networkTools.checkNetworkConnection(LookupByPatientName.this) && url.length() > 0) {
                JSONObject response = networkTools.postJsonData(LookupByPatientName.this, url, generateJson());
                try {
                    if (response != null) {
                ...
                Toast.makeText(LookupByPatientName.this, CMN_ErrorMessages.getInstance().getValue(12), Toast.LENGTH_SHORT).show();
            }
        }
            ...
            if (response.getJSONObject("Result").getInt("Code") == 0) {
                ...
                if (messagesArray.length() == 0) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(LookupByPatientName.this, "No patient found for the given Name.", Toast.LENGTH_SHORT).show();
                        }
                    });
                    return;
                }
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(isSearchPatient) {
                            ...
                            ArrayAdapter _adapter = new ArrayAdapter<String>(LookupByPatientName.this,
                            ...
                            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                                if(i==0){
                                    ...
                                    if (Utils.viewSchedule) {
                                        Intent intent = new Intent(LookupByPatientName.this, CMN_GenericViewActivity.class);
                                        intent.putExtra("scheduling", "" + ViewScheduleActivity.jsonObject);
                                        CMN_Preferences.setScheduleContextId(LookupByPatientName.this, lookupModels.get(_patient.getSelectedItemPosition()).getID());
                                        startActivity(intent);
                                    } else {
                                        Intent intent = new Intent(LookupByPatientName.this, CalenderActivity.class);
                                        CMN_Preferences.setScheduleContextId(LookupByPatientName.this, lookupModels.get(_patient.getSelectedItemPosition()).getID());
                                        startActivity(intent);
                                    }
    ...
    public JSONObject generateJson() {
        ...
        try {
            jsonObject.put("PatientName", "" + _patName.getText().toString().trim());
        } catch (JSONException e) {
            e.getMessage();
        }
    ...
    private void showAlert(String message) {
        final AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(LookupByPatientName.this, android.R.style.Theme_Dialog));
