...
public class AssignPACUNursesActivityCMN extends CMN_AppBaseActivity implements NetworkCallBack {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            networkAsyncTask1.startNetworkCall(ThreadTaskIds.GET_ALL_PROVIDERS, AssignPACUNursesActivityCMN.this);
        ...
        try {
            networkAsyncTask2.startNetworkCall(ThreadTaskIds.GET_PATIENT_NURES_ASSIGNMENT, AssignPACUNursesActivityCMN.this);
        } catch (NetworkException e) {
            e.getMessage();
        }
        ...
        assign.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    networkAsyncTask.startNetworkCall(ThreadTaskIds.SAVE_ASSIGN_NURSE_T0_PATIENT, AssignPACUNursesActivityCMN.this);
                } catch (NetworkException e) {
                    e.getMessage();
                }
            }
...
//        inflater.inflate(R.menu.savesurveydata, menu);
    ...
    private JSONObject generateJson() {
        ...
        if (patIds.isEmpty() || patIds.length() == 0) {
            jsonObject = null;
            Toast.makeText(AssignPACUNursesActivityCMN.this, CMN_ErrorMessages.getInstance().getValue(152), Toast.LENGTH_SHORT).show();
        } else if (providers.getSelectedItemPosition() == 0) {
            Toast.makeText(AssignPACUNursesActivityCMN.this, CMN_ErrorMessages.getInstance().getValue(153), Toast.LENGTH_SHORT).show();
            jsonObject = null;
        } else {
            try {
    ...
    public Object onNetworkCall(int taskId, Object o) throws JsonParsingException, JsonParsingException {
        if (taskId == ThreadTaskIds.GET_ALL_PROVIDERS) {
            String url = MessageTabActivityCMN.CMN_SERVER_BASE_URL_DEFINE
                    + "GetAllProviders.aspx?token="+ CMN_Preferences.getUserToken(AssignPACUNursesActivityCMN.this) + "&AssignmentOption=" + assignmentsTypeModel.getAssignmentsOptions();

            NetworkTools networkTools = NetworkTools.getInstance();
            if (networkTools.checkNetworkConnection(AssignPACUNursesActivityCMN.this) && url.length() > 0) {
                JSONObject response = networkTools.getJsonData(AssignPACUNursesActivityCMN.this, url);
                try {
                    if (response != null) {
                ...
                Toast.makeText(AssignPACUNursesActivityCMN.this, CMN_ErrorMessages.getInstance().getValue(12), Toast.LENGTH_SHORT).show();
            }
        }
        if (taskId == ThreadTaskIds.GET_PATIENT_NURES_ASSIGNMENT) {
            String url = MessageTabActivityCMN.CMN_SERVER_BASE_URL_DEFINE
                    + "GetPatientNurseAssignments.aspx?token="+ CMN_Preferences.getUserToken(AssignPACUNursesActivityCMN.this);
        ...
        if (taskId == ThreadTaskIds.SAVE_ASSIGN_NURSE_T0_PATIENT) {
            String url = MessageTabActivityCMN.CMN_SERVER_BASE_URL_DEFINE
                    + "AssignNurseToPatient.aspx?token="+ CMN_Preferences.getUserToken(AssignPACUNursesActivityCMN.this) + "&assign=" + isAssign + "&AssignmentOption=" + assignmentsTypeModel.getAssignmentsOptions();

            NetworkTools networkTools = NetworkTools.getInstance();
            if (networkTools.checkNetworkConnection(AssignPACUNursesActivityCMN.this) && url.length() > 0) {
                JSONObject response = networkTools.postJsonData(AssignPACUNursesActivityCMN.this, url, jsonObject);
                try {
                    if (response != null) {
    ...
    private void parseJsonData(final JSONObject response, int id) throws JSONException {
        if (id == ThreadTaskIds.GET_ALL_PROVIDERS) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    providersNameAdapter = new ArrayAdapter<String>(AssignPACUNursesActivityCMN.this,
        ...
        if (id == ThreadTaskIds.GET_PATIENT_NURES_ASSIGNMENT) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    assignNurseToPatientAdapter = new CNM_AdapterAssignNurseToPatient(AssignPACUNursesActivityCMN.this, nurseAssignmentModels);
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    try {
//                        if (response.getJSONObject("Result").getInt("Code") == 0) {
                        AlertDialog.Builder builder1 = new AlertDialog.Builder(AssignPACUNursesActivityCMN
                                .this);
